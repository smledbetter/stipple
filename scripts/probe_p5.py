"""P5.0 verification probe — RasterScan density rendering.

Opens examples/p5_density_smoke.ipynb (10M points, 5 Gaussian clusters,
render_mode='density'). Verifies that:
  1. The widget reports a 'data-rendered' status with non-zero FPS.
  2. The active mode reads as 'density' (round-tripped from JS via trait).
  3. The canvas isn't all-background — a center-region pixel sample has
     non-trivial saturation, indicating the density texture rendered.

Pass: FPS > 55 (essentially 60), density-rendered pixels visible.
"""

from __future__ import annotations

import os
import socket
import subprocess
import sys
import time
import urllib.request
from pathlib import Path

from playwright.sync_api import sync_playwright

REPO = Path(__file__).resolve().parent.parent
TOKEN = "stipple-p5-token"
NOTEBOOK = "examples/p5_density_smoke.ipynb"


def find_free_port(start: int = 9050) -> int:
    for p in range(start, start + 50):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            if s.connect_ex(("127.0.0.1", p)) != 0:
                return p
    raise RuntimeError("no free port near 9050")


def wait_for_jupyter(port: int, timeout: float = 30.0) -> None:
    deadline = time.monotonic() + timeout
    url = f"http://127.0.0.1:{port}/api?token={TOKEN}"
    while time.monotonic() < deadline:
        try:
            with urllib.request.urlopen(url, timeout=1) as r:
                if r.status == 200:
                    return
        except Exception:
            pass
        time.sleep(0.3)
    raise RuntimeError(f"jupyter at :{port} didn't come up within {timeout}s")


def start_jupyter(port: int) -> subprocess.Popen:
    work_dir = REPO / ".jupyter-probe" / "workspaces"
    work_dir.mkdir(parents=True, exist_ok=True)
    for f in work_dir.glob("*.jupyterlab-workspace"):
        f.unlink()
    subprocess.run(
        [str(REPO / ".venv" / "bin" / "jupyter"), "trust", str(REPO / NOTEBOOK)],
        check=False,
        capture_output=True,
    )
    env = os.environ.copy()
    env["JUPYTER_DISABLE_BROWSER_OPEN"] = "1"
    return subprocess.Popen(
        [
            str(REPO / ".venv" / "bin" / "jupyter"),
            "lab",
            "--no-browser",
            f"--port={port}",
            f"--IdentityProvider.token={TOKEN}",
            "--ServerApp.disable_check_xsrf=True",
            f"--notebook-dir={REPO}",
            "--allow-root",
            f"--LabApp.workspaces_dir={work_dir}",
        ],
        cwd=str(REPO),
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
        env=env,
    )


def avg_region_rgb(img_path: Path, cx_frac: float, cy_frac: float, half_px: int = 24) -> tuple[float, float, float]:
    from PIL import Image
    img = Image.open(img_path).convert("RGB")
    w, h = img.size
    cx = int(w * cx_frac)
    cy = int(h * cy_frac)
    region = img.crop((cx - half_px, cy - half_px, cx + half_px, cy + half_px))
    px = list(region.getdata())
    n = max(1, len(px))
    rs = sum(p[0] for p in px) / n
    gs = sum(p[1] for p in px) / n
    bs = sum(p[2] for p in px) / n
    return rs, gs, bs


def run_probe(headed: bool = False) -> dict:
    port = find_free_port()
    print(f"[probe] starting jupyter on :{port}")
    jupyter = start_jupyter(port)
    result: dict = {"port": port}

    try:
        wait_for_jupyter(port)
        with sync_playwright() as pw:
            browser = pw.chromium.launch(
                headless=not headed,
                args=[
                    "--enable-unsafe-webgpu",
                    "--use-angle=metal",
                    "--enable-features=Vulkan",
                ],
            )
            ctx = browser.new_context(viewport={"width": 1400, "height": 1100})
            page = ctx.new_page()
            page.on("pageerror", lambda e: print(f"[pageerror] {e}"))
            page.on(
                "console",
                lambda m: print(f"[console] {m.type}: {m.text}")
                if m.type in ("error", "warning")
                else None,
            )

            url = f"http://127.0.0.1:{port}/lab/tree/{NOTEBOOK}?token={TOKEN}"
            page.goto(url, wait_until="domcontentloaded", timeout=30_000)
            page.wait_for_selector(".jp-Notebook", state="attached", timeout=60_000)
            page.wait_for_selector(".jp-Cell", state="attached", timeout=30_000)
            time.sleep(3.0)

            page.evaluate("() => document.querySelector('.jp-Notebook')?.focus()")
            time.sleep(0.3)
            page.keyboard.press("Meta+Shift+C")
            page.wait_for_selector(".lm-CommandPalette-input", timeout=5_000)
            page.fill(".lm-CommandPalette-input", "Run All Cells")
            time.sleep(0.4)
            page.keyboard.press("Enter")

            page.wait_for_function(
                """() => {
                    const outs = Array.from(document.querySelectorAll('.jp-OutputArea-output'));
                    return outs.some(o => /render_mode\\s*:\\s*density/.test(o.textContent)
                                          && /FPS\\s*:\\s*[\\d.]+/.test(o.textContent));
                }""",
                timeout=180_000,
            )
            # Give the density build + first frame an extra beat after the FPS banner.
            time.sleep(1.5)

            outputs = page.locator(".jp-OutputArea-output").all_text_contents()
            banner = next((o for o in outputs if "render_mode" in o), "")
            result["banner"] = banner[:300]

            import re
            m = re.search(r"FPS\s*:\s*([\d.]+)", banner)
            if m:
                result["fps"] = float(m.group(1))
            m = re.search(r"frame_ms\s*:\s*([\d.]+)", banner)
            if m:
                result["frame_ms"] = float(m.group(1))
            m = re.search(r"render_mode\s*:\s*(\w+)", banner)
            if m:
                result["render_mode"] = m.group(1)
            m = re.search(r"rows\s*:\s*([\d,]+)", banner)
            if m:
                result["rows"] = int(m.group(1).replace(",", ""))

            canvas = page.locator(
                '.jp-OutputArea-output canvas[data-stipple-role="render"]'
            ).first
            canvas.scroll_into_view_if_needed(timeout=5_000)
            shot = REPO / "scripts" / "p5-canvas.png"
            canvas.screenshot(path=str(shot))
            result["screenshot"] = str(shot)

            rgb_center = avg_region_rgb(shot, 0.5, 0.5)
            rgb_corner = avg_region_rgb(shot, 0.05, 0.05)
            result["rgb_center"] = tuple(round(v, 1) for v in rgb_center)
            result["rgb_corner"] = tuple(round(v, 1) for v in rgb_corner)

            browser.close()

    finally:
        jupyter.terminate()
        try:
            jupyter.wait(timeout=5)
        except subprocess.TimeoutExpired:
            jupyter.kill()

    return result


def main() -> int:
    headed = "--headed" in sys.argv
    r = run_probe(headed=headed)
    print("\n" + "=" * 60)
    print("P5.0 PROBE — RasterScan density rendering")
    for k in ("render_mode", "rows", "fps", "frame_ms", "rgb_center", "rgb_corner", "screenshot"):
        if k in r:
            print(f"  {k:<12}: {r[k]}")
    print("=" * 60)

    fps = r.get("fps") or 0.0
    rgb_center = r.get("rgb_center") or (0.0, 0.0, 0.0)
    rgb_corner = r.get("rgb_corner") or (255.0, 255.0, 255.0)
    mode = r.get("render_mode") or ""
    rows = r.get("rows") or 0

    mode_ok = mode == "density"
    rows_ok = rows == 10_000_000
    fps_ok = fps > 55.0
    # Center has clusters (any of viridis green/yellow/blue should boost saturation),
    # corner should be near-empty dark.
    center_bright = sum(rgb_center) > 60.0
    corner_dark = sum(rgb_corner) < 90.0  # cluster background is #0d1117 ≈ (13,17,23)

    print(f"  mode == density: {mode_ok}")
    print(f"  rows == 10M: {rows_ok}")
    print(f"  FPS > 55: {fps_ok} ({fps:.1f})")
    print(f"  center has density pixels: {center_bright} (rgb_center={rgb_center})")
    print(f"  corner is empty: {corner_dark} (rgb_corner={rgb_corner})")

    ok = mode_ok and rows_ok and fps_ok and center_bright and corner_dark
    return 0 if ok else 1


if __name__ == "__main__":
    sys.exit(main())
